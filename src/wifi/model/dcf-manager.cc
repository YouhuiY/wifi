 ...
 * published by the Free Software Foundation;sssssssssss
 *
...
{
  //std::cout<<tempCollisionDC<<std::endl;
  if(m_cwMax > 9)
  {
      uint32_t tempCWDelta;
     if((lastTime - beforeLastTime) != 0)
         tempCWDelta = m_cw + tempCollisionDC +(Simulator::Now().GetMilliSeconds () - lastTime)*(tempCollisionDC - collisionDCStore)/(lastTime - beforeLastTime);
      else
         tempCWDelta = m_cw;
    if(tempCollisionDC == 0)
    {
      //m_cw = std::max (m_cw/2, m_cwMin);
      m_cw = m_cwMin;
    }
    else
    {

     // m_cw = std::min (tempCWDelta, m_cwMax);
      m_cw = m_cwMin;
    }
  ...
  //m_cw = std::min (m_cwMin ,m_cwMax);
}

//my reset function
/*void DcfState::ResetCwDC(uint32_t tempCollisionDC)
{
   //std::cout<<tempCollisionDC<<std::endl;
    m_cw = std::min (m_cwMin + 4,m_cwMax);
  //m_cw = m_cwMin;
}*/
void
