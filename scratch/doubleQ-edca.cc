...
int main (int argc, char *argv[])
{
    ...
    for(int v = 1; v < 64; v++)
    {
        for(int i = 0; i < 1; i++) 
        {
          ...
          dest_DC.SetTos (0xc0); //AC_BE
          UdpClientHelper Client_DC (dest_DC);
          Client_DC.SetAttribute ("MaxPackets", UintegerValue (4294967295u));
          Client_DC.SetAttribute ("Interval", TimeValue (Time ("0.00001"))); //packets/s
          ...
          Client_DT.SetAttribute ("Interval", TimeValue (Time ("0.00001"))); //packets/s
          ...
          outData.open("/home/youhui/Downloads/ns-allinone-3.26/ns-3.26/scratch/doubleQ-same.dat", std::ios::app);
          if (!outData)
          {
         ...
         {
            outData  << v << " " << throughput_DC <<" " << throughput_DT  << std::endl;
            outData.close();
         }
